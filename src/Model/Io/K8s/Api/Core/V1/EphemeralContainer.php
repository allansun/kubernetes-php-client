...
/**
 * An EphemeralContainer is a special type of container which doesn't come with any
 * resource or scheduling guarantees but can be added to a pod that has already
 * been created. They are intended for user-initiated activities such as
 * troubleshooting a running pod. Ephemeral containers will not be restarted when
 * they exit, and they will be killed if the pod is removed or restarted. If an
 * ephemeral container causes a pod to exceed its resource allocation, the pod may
 * be evicted. Ephemeral containers are added via a pod's ephemeralcontainers
 * subresource and will appear in the pod spec once added. No fields in
 * EphemeralContainer may be changed once added. This is an alpha feature enabled
