...
class Node extends \KubernetesRuntime\AbstractAPI
{
     ...
     * @return TheNode|mixed
     */
    public function replace($name, \Kubernetes\Model\Io\K8s\Api\Core\V1\Node $Model, array $queries = [])
    {
        return $this->parseResponse(
     ...
     * @return TheNode|mixed
     */
    public function replaceStatus($name, \Kubernetes\Model\Io\K8s\Api\Core\V1\Node $Model, array $queries = [])
    {
        return $this->parseResponse(
        	$this->client->request('put',
        		"/api/v1/nodes/{$name}/status"
